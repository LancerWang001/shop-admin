...
import * as menuApi from '@/api/rules'
import { ElMessage } from 'element-plus'
import type { Rule } from '@/api/types/rules'
...
const formData = ref<{ path: number[] } & Omit<Rule, 'id' | 'children' | 'is_del' | 'path'>>({
  auth_type: 1,
  menu_name: '',
  pid: 0,
  params: '',
...
const handleSubmit = async () => {
  ...
  if (!valid) return
  if (props.ruleId) {
    await menuApi.editMenu(props.ruleId, formData.value)
  } else {
